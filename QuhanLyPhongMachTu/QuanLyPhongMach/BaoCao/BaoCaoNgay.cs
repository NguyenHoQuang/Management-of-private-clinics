...
{
    public partial class BaoCaoNgay : UserControl
    {
        private int dem = 2;
        ...
        {
            //kiem tra de trong
            if (cbo_thang.Text == ""|| cbo_thang.Text==null)
                return 1;
            //
            try
            {
        ...
        public void ThemHang(int n, string ngaykham, string sobenhnhan, string doanhthu, string tyle)
        {
            ...
            temp.Cells.Add(cell);
            cell = new DataGridViewTextBoxCell();
            cell.Value = sobenhnhan;
            temp.Cells.Add(cell);
            cell = new DataGridViewTextBoxCell();
            cell.Value = string.Format("{0:0,0}", (int)float.Parse(doanhthu));
            temp.Cells.Add(cell);
            cell = new DataGridViewTextBoxCell();
            cell.Value = tyle + " %";
            temp.Cells.Add(cell);
        ...
        private void BaoCaoNgay_EnabledChanged(object sender, EventArgs e)
        {
            try
            {
                if (Enabled == true)
        ...
        private void BaoCaoNgay_VisibleChanged(object sender, EventArgs e)
        {
            try
            {
        ...
        private void rd_xuat_Click(object sender, EventArgs e)
        {
            if (dgv_baocao.Rows.Count > 1)
            {
                ...
                pdf.UseFont(fnt, 25);
                pdf.ShowUnicodeTextAt(lbl_baocaongay.Location.X - 70, lbl_baocaongay.Location.Y + 20, lbl_baocaongay.Text);
                pdf.UseFont(fnt, 12);
                pdf.ShowUnicodeTextAt(lbl_thang.Location.X - 70, lbl_thang.Location.Y + 10, lbl_thang.Text);
                pdf.ShowUnicodeTextAt(cbo_thang.Location.X - 70, cbo_thang.Location.Y + 10, cbo_thang.Text);
                int n = dgv_baocao.Rows.Count;
                ...
                pdf.SetTableColumnSize(hoadon, 4, 70);
                pdf.SetCellTableText(hoadon, 0, 0, "STT");
                pdf.SetCellTableText(hoadon, 1, 0, "Ngày khám");
                pdf.SetCellTableText(hoadon, 2, 0, "Số bệnh nhân");
                pdf.SetCellTableText(hoadon, 3, 0, "Doanh thu");
                pdf.SetCellTableText(hoadon, 4, 0, "Tỷ lệ");
                ...
                foreach (DataGridViewRow row in dgv_baocao.Rows)
                {
                    if (row.Cells[0].Value != null)
                    {
                        ...
                        pdf.SetCellTableText(hoadon, 1, temp, string.Format("{0:dd/MM/yyyy}", row.Cells[1].Value.ToString()));
                        ...
                        pdf.SetCellTableTextAlign(hoadon, 1, temp, TextAlign.taRight);
                        pdf.SetCellTableTextAlign(hoadon, 2, temp, TextAlign.taRight);
                        pdf.SetCellTableTextAlign(hoadon, 3, temp, TextAlign.taCenter);
                        pdf.SetCellTableTextAlign(hoadon, 4, temp, TextAlign.taRight);
                        temp++;
                    }
                }
                pdf.ShowTable(hoadon, dgv_baocao.Location.X + 45, dgv_baocao.Location.Y, 1, 1);
                ...
                timer1_Tick(sender, e);
                pdf.SaveToFile(lbl_path.Text + "\\bao cao ngay (" + string.Format("{0:MM-yyyy}", NgayDauThang(cbo_thang.Text)) + ").pdf", true);
                rd_xuat_MouseHover(sender, e);
            }
            else
            {
        ...
        private void BaoCaoNgay_Load(object sender, EventArgs e)
        {
